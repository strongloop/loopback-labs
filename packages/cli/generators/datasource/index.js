...
module.exports = class DataSourceGenerator extends ArtifactGenerator {
  ...
  promptArtifactName() {
    ...
    return this.prompt(prompts).then(props => {
      ...
      this.artifactInfo.artifactDir = this.destinationPath(
        this.artifactInfo.outDir,
        utils.urlSlug(this.artifactInfo.name)
      );
      return props;
    });
  }
  ...
  promptConnectorConfig() {
    ...
    debug(`connector setting questions - ${prompts}`);
  ...
  // Install connector package -- no prompt, just install.
  installPackage() {
    debug('install connector package');
    const connector = connectors[this.artifactInfo.connector];
    if (!connector || !connector.package) return;

    const npmModule =
      connector.package.name +
      `${connector.package.version ? '@' + connector.package.version : ''}`;

    debug(`npmModule - ${npmModule}`);

    this.npmInstall([npmModule], {save: true});
  }

  /**
   * Create the DataSource JSON file. Each DataSource gets it's own file.
   */
  createDataSourceJSON() {
    debug(`create datasource json file`);
    // JSON file name
    this.artifactInfo.jsonFile = `${this.artifactInfo.name}.datasource.json`;
    // Resolved output file path
    const outFile = this.destinationPath(
      this.artifactInfo.artifactDir,
      this.artifactInfo.jsonFile
    );

    // Data to save to DataSource File
    ...
    debug(`datasource information going to file: ${ds}`);

    // Write JSON file.
    this.fs.writeJSON(outFile, ds);
  }

  /**
   * Create the base DataSource Class and the actual class extending the base
   * that a user can modify / extend.
   */
  createDataSourceClass() {
    debug(`create datasource class files`);
    // Class Names Including Suffix
    this.artifactInfo.className = `${utils.toClassName(
      this.artifactInfo.name
    )}DataSource`;
    this.artifactInfo.baseClassName = `${this.artifactInfo.className}Base`;

    debug(`base class name: ${this.artifactInfo.baseClassName}`);
    debug(`class name: ${this.artifactInfo.className}`);

    // File Names for both classes (not full path)
    const fileName = utils.kebabCase(this.artifactInfo.name);
    this.artifactInfo.baseFileWithoutExt = `${fileName}.datasource.base`;
    this.artifactInfo.baseClassFile = `${
      this.artifactInfo.baseFileWithoutExt
    }.ts`;
    this.artifactInfo.classFile = `${fileName}.datasource.ts`;

    debug(`base class file: ${this.artifactInfo.baseClassFile}`);
    debug(`class file: ${this.artifactInfo.classFile}`);

    // Templates
    const classTemplatePath = this.templatePath('datasource.ts.ejs');
    const baseClassTemplatePath = this.templatePath('datasource.base.ts.ejs');

    // Resolved destination output paths
    const baseClassDest = this.destinationPath(
      this.artifactInfo.artifactDir,
      this.artifactInfo.baseClassFile
    );
    const classDest = this.destinationPath(
      this.artifactInfo.artifactDir,
      this.artifactInfo.classFile
    );

    debug(`base class file destination: ${this.artifactInfo.baseClassDest}`);
    debug(`class file destination: ${this.artifactInfo.classDest}`);

    // Copy Templates
    this.fs.copyTpl(baseClassTemplatePath, baseClassDest, this.artifactInfo);
    this.fs.copyTpl(classTemplatePath, classDest, this.artifactInfo);
  }

  updateIndexFile() {
    index(this.artifactInfo.outDir, {prefix: '.datasource'});
  }
